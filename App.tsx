...
const App: React.FC = () => {
  ...
  const [closingCostsMode, setClosingCostsMode] = useState<'percent' | 'currency'>('percent');
  ...
  const results = useMemo((): CalculationResult => {
    ...
    const closingCostsVal = closingCostsMode === 'percent' 
    ...
    const totalClosingCosts = closingCostsVal + costs.proratedPropertyTaxes + costs.otherFees;
    
    const sellingExpenses = totalCommissions + totalClosingCosts + (costs.repairCredits || 0);
    ...
    return {
      ...
      breakdown: [
        ...
        { name: 'Net Proceeds', value: Math.max(0, netProceeds), fill: '#ffffff10' },
      ]
    };
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-500 relative overflow-x-hidden ${theme === 'dark' ? 'text-slate-100' : 'text-slate-900'}`}>
      ...
      <nav className="h-16 lg:h-20 shrink-0 px-6 lg:px-12 flex justify-between items-center z-50 bg-transparent border-b border-black/5 dark:border-white/5">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 lg:w-9 lg:h-9 bg-emerald-500 rounded-xl flex items-center justify-center shadow-lg shadow-emerald-500/20">
            <i className="fas fa-chart-line text-slate-950 text-xs lg:text-sm"></i>
          </div>
          ...
          <div className="h-4 w-[1px] bg-slate-300 dark:bg-slate-800 mx-3 hidden sm:block"></div>
          <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest hidden sm:block">Net Recovery Sheet</span>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <i className={`fas ${theme === 'dark' ? 'fa-sun text-orange-400' : 'fa-moon text-indigo-400'} text-sm`}></i>
          </button>
      ...
      <main className="flex-1 flex flex-col lg:flex-row p-4 lg:p-10 gap-8 max-w-[1700px] mx-auto w-full relative z-10">
        ...
        {/* Input Sidebar: Left aligned for focus */}
            ...
            <div className="space-y-6">
              <div>
                <h3 className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                  <i className="fas fa-house text-[8px]"></i> Core Asset Info
                </h3>
                <InputGroup 
                  label="Listing Sale Price" value={costs.salePrice} type="currency" icon="fa-tag"
                  onChange={(v) => setCosts({...costs, salePrice: v})}
                />
                <InputGroup 
                  label="Original Purchase" value={costs.purchasePrice} type="currency" icon="fa-history"
              ...
              <div className="pt-6 border-t border-slate-200 dark:border-white/5">
                <h3 className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                  <i className="fas fa-landmark text-[8px]"></i> Debt Obligations
                </h3>
                <InputGroup 
                  label="Mortgage Balance" value={costs.mortgagePayoff} type="currency" icon="fa-building-columns"
                ...
                {!showOtherLiens && costs.otherLiens === 0 ? (
                  ...
                  >
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest">Add Liens / Taxes</span>
                  </button>
                ) : (
                  <div className="animate-in slide-in-from-top-2">
                    <InputGroup 
                      label="Other Recorded Liens" value={costs.otherLiens} type="currency" icon="fa-file-invoice-dollar"
              ...
              <div className="pt-6 border-t border-slate-200 dark:border-white/5">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest flex items-center gap-2">
                    <i className="fas fa-handshake text-[8px]"></i> Commissions & Closing
                  </h3>
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <InputGroup 
                    label="Listing %" value={costs.agentCommissionPercent} type="percent"
                    onChange={(v) => setCosts({...costs, agentCommissionPercent: v})}
                  />
                  <InputGroup 
                    label="Buyer's %" value={costs.buyerAgentCommissionPercent} type="percent"
                    onChange={(v) => setCosts({...costs, buyerAgentCommissionPercent: v})}
                  />
                </div>
                
                <div className="mt-4">
                  <div className="flex justify-between items-end mb-2 px-1">
                    <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Title & Closing</label>
                  ...
                  <InputGroup 
                    label="" value={costs.closingCostsPercent} type={closingCostsMode} icon="fa-shield-halved" hideLabel
          ...
          <div className="glass rounded-[3.5rem] p-10 lg:p-20 flex flex-col justify-center relative overflow-hidden border border-white/10 shadow-2xl min-h-[500px] lg:min-h-0">
            <div className="relative z-10 w-full max-w-4xl">
              ...
              <div className="mb-10 lg:mb-16">
                <h2 className={`font-light tracking-tighter leading-none flex items-baseline flex-wrap gap-2 lg:gap-4 transition-all duration-300 
                  ...
                  <span className="break-words">{results.netProceeds.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                </h2>
                <div className="h-1.5 w-24 bg-gradient-to-r from-emerald-500 to-transparent mt-4 rounded-full"></div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8 mb-12 lg:mb-16">
                <div className="group">
                  <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 group-hover:text-emerald-500 transition-colors">Equity Growth</p>
                  <p className={`text-4xl lg:text-5xl font-light ${results.appreciation >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600'}`}>
                    <span className="text-lg opacity-40 mr-1">{results.appreciation >= 0 ? '+' : ''}$</span>
                    {Math.abs(results.appreciation).toLocaleString(undefined, { maximumFractionDigits: 0 })}
                  </p>
                </div>
                <div className="group">
                  <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 group-hover:text-orange-400 transition-colors">Total Expenses</p>
                  <p className="text-4xl lg:text-5xl font-light text-orange-600 dark:text-orange-400">
                    <span className="text-lg opacity-40 mr-1">-$</span>{results.sellingExpenses.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                  </p>
                </div>
                <div className="group hidden xl:block">
                  <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 group-hover:text-indigo-400 transition-colors">Debt Settled</p>
                  <p className="text-4xl lg:text-5xl font-light text-indigo-500">
                    <span className="text-lg opacity-40 mr-1">-$</span>{results.debtPayoff.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-center">
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Expert Note</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400 max-w-[320px] leading-relaxed mt-1">
                      Selling costs represent approx. <b>{((results.sellingExpenses / (costs.salePrice || 1)) * 100).toFixed(1)}%</b> of your final sale price.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className={`absolute -bottom-10 -right-20 pointer-events-none select-none transition-opacity duration-1000 ${theme === 'dark' ? 'opacity-[0.04]' : 'opacity-[0.05]'}`}>
              <i className="fas fa-house-chimney-window text-[600px] lg:text-[850px] text-emerald-500 rotate-[-12deg]"></i>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="glass rounded-3xl p-8 border border-white/5">
              <div className="flex justify-between items-start mb-4">
                ...
                <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Net Ratio</span>
              </div>
              <p className="text-3xl font-light mb-1">
                {((results.netProceeds / (costs.salePrice || 1)) * 100).toFixed(1)}%
              </p>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">of Gross Sale Price</p>
            </div>
            ...
            <div className="glass rounded-3xl p-8 border border-white/5">
              <div className="flex justify-between items-start mb-4">
                ...
                <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Cost Burden</span>
              </div>
              <p className="text-3xl font-light mb-1">
                {((results.totalDeductions / (costs.salePrice || 1)) * 100).toFixed(1)}%
              </p>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Total Sunk Costs</p>
            </div>
            ...
            <div className="glass rounded-3xl p-8 border border-white/5">
              <div className="flex justify-between items-start mb-4">
                ...
                <span className="text-[10px] font-black text-orange-400 uppercase tracking-widest">Market Gains</span>
              </div>
              <p className="text-3xl font-light mb-1">
                {((results.appreciation / (costs.purchasePrice || 1)) * 100).toFixed(1)}%
              </p>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Growth Since Purchase</p>
            </div>
          </div>
