  ...
  return (
    <div
      className={`min-h-screen flex flex-col transition-colors duration-500 relative overflow-x-hidden ${
        theme === "tool-dark" ? "text-slate-100" : "text-slate-900"
      }`}
    >
      <BreakdownModal
      ...
      <nav className="h-14 sm:h-16 lg:h-20 shrink-0 px-4 sm:px-6 lg:px-12 flex justify-between items-center z-50 bg-transparent border-b border-black/5 dark:border-white/5">
        <div className="flex items-center gap-3">
          ...
          <div className="h-4 w-px bg-slate-300 dark:bg-slate-800 mx-2 sm:mx-3 hidden sm:block"></div>
        ...
        <aside className="w-full lg:w-[400px] shrink-0">
          <div className="glass !rounded-[2rem] sm:!rounded-[2.5rem] p-5 sm:p-6 lg:p-8 flex flex-col h-fit lg:sticky lg:top-10 border border-white/5 shadow-2xl overflow-hidden">
            <div className="space-y-6">
              <div>
                <h3
                  className={`text-[9px] sm:text-[10px] font-black uppercase tracking-widest mb-4 flex items-center gap-2 ${
                    theme === "tool-light" ? "text-slate-500" : "text-slate-400"
                  }`}
                >
        ...
        <section className="flex-1 flex flex-col gap-8">
          ...
          <div className="glass !rounded-[2rem] sm:!rounded-[2.5rem] lg:!rounded-[3.5rem] p-6 sm:p-8 md:p-10 lg:p-20 flex flex-col justify-center relative overflow-hidden border border-white/10 shadow-2xl min-h-[400px] sm:min-h-[500px] lg:min-h-0">
            <div className="relative z-10 w-full max-w-4xl">
              ...
              <div className="mb-8 sm:mb-10 lg:mb-16">
                <h2
                  className={`font-light tracking-tighter leading-[0.9] flex items-baseline flex-wrap gap-1 sm:gap-2 lg:gap-4 transition-all duration-300 
                  text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl
                  ${
                    theme === "tool-dark"
                      ? "text-white drop-shadow-[0_0_80px_rgba(255,255,255,0.15)]"
                      : "text-slate-900"
                  }`}
                >
                  <span className="text-emerald-500 font-normal shrink-0">
                    $
                  </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 md:gap-8 mb-6 sm:mb-8 md:mb-12 lg:mb-16">
                <div className="group">
                  <p className="text-[9px] sm:text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 group-hover:text-emerald-500 transition-colors">
                    Equity Growth
                  </p>
                  <p
                    className={`text-lg sm:text-xl md:text-2xl lg:text-3xl font-light break-words ${
                      results.appreciation >= 0
                        ? "text-emerald-600 dark:text-emerald-400"
                        : "text-rose-600"
                    }`}
                  >
                    <span className="text-xs sm:text-sm md:text-base opacity-40 mr-1">
                      {results.appreciation >= 0 ? "+" : ""}$
                    </span>
                ...
                <div className="group">
                  <p className="text-[9px] sm:text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 group-hover:text-orange-400 transition-colors">
                    Total Expenses
                  </p>
                  <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-light text-orange-600 dark:text-orange-400 break-words">
                    <span className="text-xs sm:text-sm md:text-base opacity-40 mr-1">
                      -$
                    </span>
                ...
                <div className="group block xl:block">
                  <p className="text-[9px] sm:text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-2 group-hover:text-indigo-400 transition-colors">
                    Debt Settled
                  </p>
                  <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-light text-indigo-500 break-words">
                    <span className="text-xs sm:text-sm md:text-base opacity-40 mr-1">
                      -$
                    </span>
              ...
              <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 items-start sm:items-center">
                ...
                <div className="flex gap-3 sm:gap-4 w-full sm:w-auto">
                  ...
                  <div className="flex-1 sm:flex-none">
                    ...
                    <p className="text-[10px] sm:text-[11px] md:text-xs text-slate-500 dark:text-slate-400 max-w-full sm:max-w-[320px] leading-relaxed mt-1">
            ...
            <div
              className={`absolute -bottom-10 -right-20 pointer-events-none select-none transition-opacity duration-1000 hidden md:block ${
                theme === "tool-dark" ? "opacity-[0.04]" : "opacity-[0.05]"
              }`}
            >
              <Home
                size={800}
            ...
            <div
              className={`absolute -bottom-5 -right-5 pointer-events-none select-none transition-opacity duration-1000 md:hidden ${
                theme === "tool-dark" ? "opacity-[0.02]" : "opacity-[0.03]"
              }`}
            >
              <Home
                size={300}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
            <div className="glass !rounded-2xl sm:!rounded-3xl p-6 sm:p-8 border border-white/5">
              ...
              <p className="text-base sm:text-lg md:text-xl lg:text-2xl font-light mb-1 break-words">
